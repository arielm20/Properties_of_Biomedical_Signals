%System 2 test - non-causal, linear, time varient, non-memoryless
%Ariel Motsenyat - motsenya 
%Sharon Cai - cais12

%Causality test: non-causal, the present output of this function is dependent on future input
n = -5:5;
...
y1 = system2(n,x1); %test with unit impulse
y2 = system2(n,x2); %test with step function
y3 = system2(n,x3); %test with ramp function
...
y4 = system2(n,x4); %test with step function
y5 = system2(n,x5); %test with scaled step function
y6 = system2(n,x6); %test with unit impulse
y7 = system2(n,x7); %test with unit impulse scaled by -5
y8 = system2(n,x8); %test with ramp function
y9 = system2(n,x9); %test with ramp function scaled by -1
...
y19 = system2(n,x19); 
y20 = system2(n,x20); 
y21 = system2(n,x21); %test with added functions y19 and y20
y22 = system2(n,x22); 
y23 = system2(n,x23); 
y24 = system2(n,x24); %test with added functions y22 and y23
...
%Time varience test: function is time-invarient as the output is shifted by
%the same factor as the input.
n = -5:5;
...
y10 = system2(n,x10);
y11 = system2(n,x11); %test with u[n+2]
y12 = system2(n,x12);
y13 = system2(n,x13); %test with d[n-3]
y14 = system2(n,x14);
y15 = system2(n,x15); %test with r[n+4]
...
%Memory test: output changes at timesteps other than the changed input,
%therefore, it has memory. as well, since it is noncausal it must have
%memory
n = -5:5;
...
y16 = system2(n,x16);
y17 = system2(n,x17); %test with d[n] + d[n+1]
y18 = system2(n,x18); %test with d[n+2] + d[n] + d[n-2] + d[n-4]
